...
export default function MapScreen() {
  ...
  // Drowsiness State
  const drowsinessCameraRef = useRef<CameraView>(null);
  const [cameraPermission, requestCameraPermission] = useCameraPermissions();
  const [isCheckingDrowsiness, setIsCheckingDrowsiness] = useState(false);
  const drowsinessResolveRef = useRef<((safe: boolean) => void) | null>(null);

  // Effect: when camera mounts during drowsiness check, snap + analyze
  useEffect(() => {
    if (!isCheckingDrowsiness) return;
    let cancelled = false;

    const doCheck = async () => {
      // Poll for camera ref to become available (max 3s)
      for (let i = 0; i < 15; i++) {
        if (drowsinessCameraRef.current) break;
        await new Promise(r => setTimeout(r, 200));
      }
      if (cancelled || !drowsinessCameraRef.current) {
        drowsinessResolveRef.current?.(true); // fail-open
        setIsCheckingDrowsiness(false);
        return;
      }
      ...
      try {
        ...
        const photo = await drowsinessCameraRef.current.takePictureAsync({ base64: true, quality: 0.1 });
        if (photo?.base64) {
          const res = await fetch("http://10.1.7.47:8000/api/v1/check_drowsiness", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ image_base64: photo.base64 })
          });
          const data = await res.json();
          if (data.status === "success" && data.eyes_closed === true) {
            drowsinessResolveRef.current?.(false); // NOT safe
          } else {
            drowsinessResolveRef.current?.(true); // safe
          }
        } else {
          drowsinessResolveRef.current?.(true);
        }
      } catch (e) {
        console.warn("Drowsiness check failed", e);
        drowsinessResolveRef.current?.(true); // fail-open
      }
      if (!cancelled) setIsCheckingDrowsiness(false);
    };

    doCheck();
    return () => { cancelled = true; };
  }, [isCheckingDrowsiness]);
  ...
  const { isDarkMode, isAmbulanceMode, isWomensMode, isAsthmaMode, isSirenMode, isDrowsinessMode } = useTheme();
  ...
  useEffect(() => {
    if (remoteDetection && isNavigating && location) {
      const dist = Math.round(turf.distance(
        turf.point([location.coords.longitude, location.coords.latitude]),
        turf.point(remoteDetection.event.coordinate),
        { units: 'meters' }
      ));
      Speech.speak(`${remoteDetection.type === 'incident' ? 'Incident Reported' : 'Pothole Detected'}, ${dist} meters away.`);
    }
  ...
  const startNavigationDirect = async () => {
    ...
    // -- Silent Drowsiness Check --
    if (travelProfile === 'driving' && isDrowsinessMode) {
      if (!cameraPermission?.granted) {
        await requestCameraPermission();
      }
      if (cameraPermission?.granted) {
        // Mount camera + wait for result via useEffect
        const isSafe = await new Promise<boolean>((resolve) => {
          drowsinessResolveRef.current = resolve;
          setIsCheckingDrowsiness(true);
        });

        if (!isSafe) {
          Speech.speak("Warning. Drowsiness detected. You are not ready to drive. Navigation canceled.");
          Alert.alert("⚠️ Safety Alert", "Drowsiness detected! Your eyes appear closed. Please rest before driving.");
    ...
    Speech.speak(`Starting navigation. Expected time ${getEstimatedTime(travelProfile).replace('h', 'hours').replace('m', 'minutes').replace('min', 'minutes')}.`);
  ...
  return (
    <SafeAreaProvider>
      <View style={[styles.container, { backgroundColor: isDarkMode ? '#000000' : '#ffffff' }]}>
        ...
        {/* Hidden front camera — only alive during drowsiness check */}
        {isCheckingDrowsiness && cameraPermission?.granted && (
          <CameraView facing="front" ref={drowsinessCameraRef} style={{ width: 1, height: 1, opacity: 0, position: 'absolute', zIndex: -1 }} />
        )}
